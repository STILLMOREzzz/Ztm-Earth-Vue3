...
function draw_primitive_code(viewer, code, instances, instances_outline, material=Cesium.Color.RED.withAlpha(0.2)) {
  let codeRange = decode_geosot_3d(code);

  let level = codeRange.level;
  if (codeRange.min_coord[0] >= 90 || codeRange.max_coord[0] <= -90 ||
    codeRange.min_coord[1] >= 180 || codeRange.max_coord[1] <= -180)
    return;
  ...
  if (!material){
    material = Cesium.Color.fromRandom({minimumRed:0.1,
      minimumGreen:0.1,
      minimumBlue:0.1,
      maximumRed:0.5,
      maximumGreen:0.5,
  ...
  instances_outline.push(new Cesium.GeometryInstance({
    geometry: Cesium.PolygonOutlineGeometry.createGeometry(Cesium.PolygonOutlineGeometry.fromPositions({
...
function draw_primitive_code_2d(viewer, code, instances, instances_outline, material=Cesium.Color.RED.withAlpha(0.2)) {
...
//根据二维编码画二维图
//未测
function draw_code_2d(viewer, code_2d, material=Cesium.Color.RED.withAlpha(0.2), code_collection=null, parents_grids=null) {
...
function draw_code(viewer, code, material=Cesium.Color.RED.withAlpha(0.2), code_collection=null, parents_grids=null) {
  let codeRange = decode_geosot_3d(code);
...
function draw_dynamic_code(viewer, code, material=Cesium.Color.RED.withAlpha(0.2), code_collection=null, parents_grids=null, starttime=null, stoptime=null) {
  let codeRange = decode_geosot_3d(code);
...
function draw_codes(viewer, level, lat_range, lon_range, height_range, material=Cesium.Color.WHITE.withAlpha(0), code_collection=null, parents_grids=null) {
  ...
  d_increase = 0.0000001;//微微增加一个小量
  code_min = encode_geosot_3d(level, lat_min + d_increase, lon_min + d_increase, height_min + d_increase);
  code_max = encode_geosot_3d(level, lat_max - d_increase, lon_max - d_increase, height_max - d_increase);
  ...
  for (var lonInt = lonInt_min; lonInt <= lonInt_max; lonInt += 1) {
    for (var latInt = latInt_min; latInt <= latInt_max; latInt += 1) {
      for (var heightInt = heightInt_min; heightInt <= heightInt_max; heightInt += 1) {
        ...
        for(i=0;i<level-1;i++){//用level-1而不用level，因为三个字符串的实际上第一位都是用于存正负的，之前加过了。
...
function draw_codes_2d_line(viewer, level, lon_range, lat_range, near = 0.0, far = Number.MAX_VALUE,  material = Cesium.Color.WHITE.withAlpha(0.2)) {
  ...
  d_increase = 0.0000001;//微微增加一个小量
  code_min = encode_geosot_2d(level, lat_min + d_increase, lon_min + d_increase);//左下角
  code_max = encode_geosot_2d(level, lat_max - d_increase, lon_max - d_increase);//右上角
  ...
  for (var latInt = latInt_min; latInt <= latInt_max; latInt += 1) {
    ...
    codeRange = decode_geosot_2d(code);
  ...
  lonlist.forEach(function(item, index){
    viewer.entities.add({
      polyline : {
        positions : Cesium.Cartesian3.fromDegreesArray([item, latlist[0],
          item, latlist[latlist.length-1]]),
        width : 1,
        material : material,
  ...
  latlist.forEach(function(item, index){
    ...
    for(let i = 0; i < 36; i++){
      viewer.entities.add({
        polyline : {
          positions : Cesium.Cartesian3.fromDegreesArray([lonlist[0]*(36-i)/36+lonlist[lonlist.length-1]*(i)/36, item,
            lonlist[0]*(36-1-i)/36+lonlist[lonlist.length-1]*(i+1)/36, item]),
          width : 1,
          material : material,
          distanceDisplayCondition: distanceDisplayCondition
        }
...
//利用primitive的方式画网格
function draw_primitive_codes(viewer, level, lat_range, lon_range, height_range, primitives, material=Cesium.Color.RED.withAlpha(0.2)) {
  let lonlatResolution = LON_LAT_TABLE[level];
  let heightResolution = HEIGHT_TABLE[level];
  let lat_min = lat_range ? lat_range[0] : -90;
  let lat_max = lat_range ? lat_range[1] : 90;
...
function get_children_codes(code){
  return [code+'000',code+'001',code+'010',code+'011',code+'100',code+'101',code+'110',code+'111'];
}
...
function local_lod(viewer, handler, [lat_min3, lat_max3], [lon_min3, lon_max3], [height_min, height_max],material=Cesium.Color.RED.withAlpha(0.2), code_collection=null, parents_grids=null){
  let lat_min = -90;
  let lat_max = 90;
  let lon_min = -180;
  let lon_max = 180;
  ...
  draw_codes(viewer, level, [lat_min3, lat_max3], [lon_min3, lon_max3], [height_min, height_max], material, code_collection, parents_grids);

  handler.setInputAction(function(movement) {
    for(i=0;i<code_collection.values.length;i++){
...
function local_lod_primitives(viewer, handler, [lat_min, lat_max], [lon_min, lon_max], [height_min, height_max], primitives, material=Cesium.Color.RED.withAlpha(0.2)){
  var field_view = viewer.camera.computeViewRectangle();
  let lat_min2 = Cesium.Math.toDegrees(field_view.south);
  let lat_max2 = Cesium.Math.toDegrees(field_view.north);
  let lon_min2 = Cesium.Math.toDegrees(field_view.west);
  ...
  let temp = Math.max(lon_max3 - lon_min3, lat_max3 - lat_min3);
  var level = compute_level(temp);
  draw_primitive_codes(viewer, level, [lat_min3, lat_max3], [lon_min3, lon_max3], [height_min, height_max], primitives, material);
...
function global_lod(viewer, handler,[height_min, height_max],material=Cesium.Color.RED.withAlpha(0.2), code_collection=null, parents_grids=null){
...
function local_lod_2d_line(viewer){
  ...
  draw_codes_2d_line(viewer, 15, [lon_min, lon_max], [lat_min, lat_max], style=0.99, near = 0, far = 500000);
 ...
 * @param material
 */
function draw_surrounded_obb_primitive(code,material=Cesium.Color.GREY.withAlpha(0.1)){
  let codeRange = decode_geosot_3d(code);
 ...
 * 绘制三维网格
 */
function draw_redao_3d_primitive(code,material=Cesium.Color.RED.withAlpha(0.2)){
  let codeRange = decode_geosot_3d(code);
